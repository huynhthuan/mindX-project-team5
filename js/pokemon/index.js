...
render(pokeApiUrl + `?_limit=8`, '.archivePoke__list .row');
...
function clickLoadMore() {
    ...
    render(pokeApiUrl + `?types_like=${type ? type : ''}&abilities_like=${ability.toLowerCase()}${height}${weight}&_page=${page}&_limit=8${sort.value}`, '.archivePoke__list .row', 'search');
}

let search = document.querySelector('.dropdownPokemonFillter__action');

search.innerHTML = `<button class="btn btnPokeFillter btnPokeFillterReset" onclick="resetFillter()">Reset</button>
  <button class="btn btnPokeFillter btnPokeFillterSearch" onclick="clickSearch()">
...
btnSearch.onclick = () => {
    ...
    render(pokeApiUrl + `?types_like=${type ? type : ''}&abilities_like=${ability.toLowerCase()}${height}${weight}&_limit=8`, '.archivePoke__list .row');
};
...
sort.onchange = () => {
    ...
    render(pokeApiUrl + `?types_like=${type ? type : ''}&abilities_like=${ability.toLowerCase()}${height}${weight}&_limit=8${sort.value}`, '.archivePoke__list .row');
};
